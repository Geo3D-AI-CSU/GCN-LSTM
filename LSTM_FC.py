...
def build_model(optimizer,neurons,learn_rate):
    """
    Build and configure a LSTM+FC neural network model for time series prediction.

    Parameters:
    - optimizer (tf.keras.optimizers.Optimizer): The optimizer used for model training.
    - neurons (int): The number of LSTM units and the size of the hidden layers in the network.

    Returns:
    - grid_model (tf.keras.models.Sequential): The configured LSTM+FC model.
    """
...
if __name__ == '__main__':
    outIndex = r'.\result\LSTM_FC_Index.csv'
    outPrediction = r'.\result\LSTM_FC_Prediction.csv'
    outTotalIndex = r'.\result\LSTM_FC_Total_Index.csv'
    outDataArr = []
    totalPrediction = []
    totalOriginal = []
    outPredictionArr = []
    ...
    for i in range(0, 16):
        ...
        # # Save the model
        my_model.save(r"./model/LSTM_FC.h5")
        # # Load the pre-trained model
        # my_model = tf.keras.models.load_model(r"E:\zhengwen\GCN-LSTM-Master\models\well-1\LSTM_FC.h5",
        #                                       compile=False)
